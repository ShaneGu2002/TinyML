"""Hello-Edge style two-stage hyperparameter sweep for fully-connected DNN.
...
final dnn_S / dnn_M / dnn_L model.

Outputs land under ``--output-dir`` (default ``sweep_artifacts/``)::

    sweep_artifacts/
    ├── sweep_results_dnn.csv       # every candidate, both stages
    ├── dnn_S/{best.keras, report.json}
    ├── dnn_M/{best.keras, report.json}
    └── dnn_L/{best.keras, report.json}
"""
...
# Same budget tiers as sweep_ds_cnn.py so the S/M/L comparison is apples-to-apples.
...
CSV_COLUMNS = [
    "tier", "stage", "rank",
    "layers", "units", "dropout", "mfcc_bins",
...
def sample_config(rng: random.Random) -> Dict:
    return {
        ...
        "units":     rng.choice(SEARCH_SPACE["units"]),
...
def config_key(cfg: Dict) -> Tuple:
    return (cfg["layers"], cfg["units"], cfg["dropout"], cfg["mfcc_bins"])
...
def make_csv_row(tier: str, stage: int, rank: int, cfg: Dict, cost: Dict[str, int],
    ...
    return {
        ...
        "layers": cfg["layers"], "units": cfg["units"],
        "dropout": cfg["dropout"], "mfcc_bins": cfg["mfcc_bins"],
...
def main() -> None:
    ...
    csv_path = args.output_dir / "sweep_results_dnn.csv"
    ...
    for tier_idx, tier in enumerate(args.tiers):
        budget = BUDGETS[tier]
        print(f"\n=== DNN Tier {tier}: <= {budget['memory_bytes']//1024} KB, "
        ...
        for i, (cfg, cost) in enumerate(candidates, start=1):
            print(f"\n[DNN {tier} S1 {i}/{len(candidates)}] cfg={cfg}  "
        ...
        if not stage1_results:
            print(f"[DNN {tier}] no successful Stage-1 runs, skipping tier")
            continue

        stage1_results.sort(key=lambda item: item[2], reverse=True)
        top_k = stage1_results[: args.top_k]
        print(f"\n[DNN {tier}] Stage-1 top {len(top_k)}:")
        ...
        for rank, (cfg, cost, _) in enumerate(top_k, start=1):
            cand_dir = args.output_dir / f"dnn_{tier}_top{rank}"
            ...
            print(f"\n[DNN {tier} S2 {rank}/{len(top_k)}] cfg={cfg}")
        ...
        if not stage2_results:
            print(f"[DNN {tier}] no successful Stage-2 runs, skipping tier")
            continue
        ...
        final_dir = args.output_dir / f"dnn_{tier}"
        ...
        summary[tier] = report
        print(f"\n[DNN {tier}] BEST cfg={best_cfg}  val={best_val:.4f}  test={best_test['accuracy']:.4f}")

    print("\n=== DNN sweep complete ===")
    print(json.dumps(summary, indent=2))
    (args.output_dir / "summary_dnn.json").write_text(json.dumps(summary, indent=2))
